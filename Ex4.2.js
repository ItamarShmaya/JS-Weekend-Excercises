...
function tribonacci(n, signature) {
  if( isValidNumber(n) && isValidSignature(signature) ) {
    let tribonacciArr = signature.slice();
    for(let i = 0; i < n - 3; i++) {
      let first = tribonacciArr[i];
      let second = tribonacciArr[i+1];
      let third = tribonacciArr[i+2];
      tribonacciArr.push(first + second + third); 
    }
...
function isValidSignature(signature) {
  return ( signature[0] === 0 && signature[1] === 0 && signature[2] === 1  ) || 
         ( signature[0] === 1 && signature[1] === 1 && signature[2] === 1 );
} 

// console.log(tribonacci(8, [1,1,1]));