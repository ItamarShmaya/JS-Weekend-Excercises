...
function maskify(info) {
  const lowerCaseInfo = info.toString();
  let maskedInfo = "";
  for(let i = 0; i < lowerCaseInfo.length - 4; i++) {
    maskedInfo += "#";
  }
  return maskedInfo + lowerCaseInfo.slice(maskedInfo.length);
}

console.log(maskify("as123asdsg5"));